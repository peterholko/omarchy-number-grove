...
function create(grade, total, seed) {
  return { grade: grade, total: Math.max(1, Math.min(50, total)), seed: seed >>> 0,
    phase: "waiting", answered: 0, correct: 0, hearts: 3, score: 0, streak: 0,
    player: HOME, bugs: [], stones: [], tiles: [], shield: 0,
...
function verdict(state, result) {
  ...
  if (!result || result.ok !== true) {
    ...
    if (error === "too_fast") {
      ...
      s.phase = "error"; s.message = "The answer could not be checked. Try a new round."
    }
    return s
  }
