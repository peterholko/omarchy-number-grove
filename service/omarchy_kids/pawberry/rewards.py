...
DEFAULTS = {"enabled": False, "backend": "legacy", "minutes_per_problem": 1, "daily_cap_minutes": 30}
...
def valid_patch(patch):
    ...
    for key, value in patch.items():
        if key == "backend":
            if value not in ("legacy", "platform"):
                return False
        elif key == "enabled":
...
class Rewards:
    ...
    def status(self, uid, settings):
        platform = self.host.platform.status(uid, "peterholko.pawberry")
        if settings["backend"] == "platform":
            return {**platform, "enabled": settings["enabled"],
                "legacy_settings": {key: settings[key] for key in ("minutes_per_problem", "daily_cap_minutes")},
                "active": settings["enabled"] and platform["active"],
                "reason": platform["reason"] if settings["enabled"] else "disabled",
                "backends": {"platform": platform, "legacy": {"available": self.account(uid) is not None}}}
        ...
        return {**settings, "available": account is not None, "active": not reason, "reason": reason,
            "backends": {"platform": platform, "legacy": {"available": account is not None}},
    ...
    def settle(self, uid, receipt, settings):
        if receipt.get("backend") == "platform":
            import pwd
            return self.host.platform.settle(pwd.getpwuid(uid).pw_name, "peterholko.pawberry", receipt)
        account = self.account(uid)
